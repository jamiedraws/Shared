...
const enableInertSupport = () => {
    if (!("inert" in HTMLElement.prototype)) {
        ...
        window.addEventListener("load", function () {
             ...
             */
            const dispatchTabEvent = function (opt_shiftKey) {
                var ev = null;
                try {
             ...
             */
            const targetForEvent = function (event) {
                var p = event.path;
            ...
            const updateFocusedShadowRoot = function (root) {
                ...
                if (focusedShadowRoot) {
                    if (!(focusedShadowRoot instanceof window.ShadowRoot)) {
                        throw new Error(
                            "not shadow root: " + focusedShadowRoot
                        );
                    }
                    focusedShadowRoot.removeEventListener(
                        "focusin",
                        shadowFocusHandler,
                        true
                    ); // remove
             ...
             * @param {!Event} ev
             */
            const shadowFocusHandler = function (ev) {
             ...
             */
            const sharedFocusHandler = function (target) {
            ...
            document.body.addEventListener(
                ...
                function (ev) {
                    ...
                    updateFocusedShadowRoot(
                        target == ev.target ? null : findShadowRoot(target)
                    );
...
export default enableInertSupport;
